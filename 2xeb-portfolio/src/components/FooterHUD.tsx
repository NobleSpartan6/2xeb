      ...
      <div className={`fixed bottom-0 left-0 right-0 z-[75] transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)]`}>
        ...
        <div className="bg-[#080808] border-t border-[#262626]">
          ...
          <div className="max-w-[1800px] mx-auto px-4 sm:px-6 lg:px-12">
            ...
            <div className="min-h-14 py-2 text-[10px] uppercase tracking-[0.15em] font-mono text-[#737373]">
              ...
              <div className="hidden sm:grid sm:grid-cols-3 items-center gap-4">
                ...
                <div className="flex gap-6 md:gap-8 justify-center">
                  <span className="flex items-center gap-2 hover:text-[#06B6D4] transition-colors cursor-pointer py-2" onMouseEnter={() => setFocusedDiscipline(ConsoleLane.CODE)} onMouseLeave={() => setFocusedDiscipline(null)}>
                    <span className="w-2 h-2 bg-[#06B6D4]"></span>
                    <span>Software</span>
                  </span>
                  <span className="flex items-center gap-2 hover:text-[#84CC16] transition-colors cursor-pointer py-2" onMouseEnter={() => setFocusedDiscipline(ConsoleLane.VISION)} onMouseLeave={() => setFocusedDiscipline(null)}>
                    <span className="w-2 h-2 bg-[#84CC16]"></span>
                    <span>ML / AI</span>
                  </span>
                  <span className="flex items-center gap-2 hover:text-[#F59E0B] transition-colors cursor-pointer py-2" onMouseEnter={() => setFocusedDiscipline(ConsoleLane.DESIGN)} onMouseLeave={() => setFocusedDiscipline(null)}>
                ...
                <div className="flex items-center justify-end">
                  {isMLLabPage ? (
                    <span className="text-[#2563EB] inline-flex items-center gap-2">
                      <div className="w-5 h-5 bg-[#2563EB]/10 border border-[#2563EB]/30 grid place-items-center">
                        <span className="text-[#2563EB] font-bold text-[8px] font-space-grotesk tracking-tight">EB</span>
                      </div>
                      EB Active
                    </span>
                  ) : (
                    <button onClick={() => setIsAgentOpen(!isAgentOpen)} className={`flex items-center gap-2 px-4 py-2 transition-all border ${isAgentOpen ? 'bg-[#2563EB] border-[#2563EB] text-white' : 'bg-transparent border-[#333] text-[#a3a3a3] hover:border-[#2563EB] hover:text-white hover:bg-[#2563EB]/10'}`}>
                    <span>{isAgentOpen ? 'CLOSE' : 'ASK'}</span>
                    <div className={`w-5 h-5 border grid place-items-center transition-colors ${isAgentOpen ? 'bg-white/10 border-white/30' : 'bg-[#0A0A0A] border-[#333]'}`}>
                      <span className={`font-bold text-[8px] font-space-grotesk tracking-tight ${isAgentOpen ? 'text-white' : 'text-[#2563EB]'}`}>EB</span>
                    </div>
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className={`w-3 h-3 transition-transform duration-300 ${isAgentOpen ? 'rotate-180' : ''}`}><path strokeLinecap="round" strokeLinejoin="round" d="M4.5 15.75l7.5-7.5 7.5 7.5" /></svg>
                    </button>
                  )}
                </div>
              </div>
