...
- **Input**: `{ question, context, model?, stream? }`
  - `model`: Specific model ID (e.g., `"openai/gpt-oss-20b"`)
...
- **Available Models**:
  ...
  |----------|-------------|---------|
  | `openai/gpt-oss-20b` | 1,000 | No |
  | `qwen/qwen3.6-27b` | 1,000 | No |
  | `openai/gpt-oss-120b` | 1,000 | **Yes** |
...
Defined in `sql/2026-09-08_posts.sql`. Columns: `id bigint identity`, `slug` (unique, `^[a-z0-9][a-z0-9-]*# Supabase Directory - CLAUDE.md
...
), `title?`, `body` (markdown), `excerpt?`, `kind` (note|work), `discipline?`, `status` (draft|unlisted|published), `published_at?`, `created_at`, `updated_at`.

- Trigger `posts_before_write`: bumps `updated_at`, stamps `published_at` the first time a piece goes public, names untitled pieces `YYYY-MM-DD-xxxxxx` when no slug is given.
- Audit trigger `audit_posts` → `audit_log`.
- RLS: `anon`/`authenticated` select `status = published`; `is_admin()` full access.
- `get_post(p_slug)` (security definer): one row if published **or unlisted** — the only way an unlisted piece is readable, so unlisted pieces cannot be enumerated.
